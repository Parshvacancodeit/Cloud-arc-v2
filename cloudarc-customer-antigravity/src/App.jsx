  ...
  return (
    <div>
      <div className="page-header">
        ...
        <div className="page-subtitle">Delivering to your address</div>
      </div>

      <div style={{ paddingTop: 8, paddingBottom: 24 }}>
        {error && <div style={{ margin: '0 20px 16px', padding: '12px 14px', background: 'rgba(239,68,68,0.1)', border: '1px solid rgba(239,68,68,0.3)', borderRadius: 12, fontSize: 13, color: '#EF4444', display: 'flex', alignItems: 'center', gap: 8 }}><FiAlertTriangle /> {error}</div>}

        {[['name', 'Full Name *', 'text', 'Rahul Sharma'], ['phone', 'Phone Number *', 'tel', '+91 98765 43210'], ['address', 'Delivery Address *', 'text', '123, MG Road, Flat 4A']].map(([field, label, type, placeholder]) => (
          <div key={field} className="form-field">
            <label>{label}</label>
            <input type={type} name={field} value={form[field]} onChange={handleChange} placeholder={placeholder} />
          </div>
        ))}

        <div className="form-field">
          <label>Special Instructions</label>
          <textarea name="notes" value={form.notes} onChange={handleChange} rows={3} placeholder="No onions, extra spicy..." />
        </div>

        <div style={{ margin: '0 20px 20px', padding: '14px', background: 'rgba(0,173,181,0.08)', border: '1px solid rgba(0,173,181,0.2)', borderRadius: 12 }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: 15, fontWeight: 800 }}>
            <span>Order Total</span>
            <span style={{ color: '#00ADB5' }}>₹{total}</span>
          </div>
          <div style={{ fontSize: 12, color: '#94A3B8', marginTop: 4 }}>Cash on delivery</div>
        </div>

        <button className="primary-btn" onClick={handleOrder} disabled={loading}>
          {loading ? 'Placing Order...' : <><FiPackage style={{ marginRight: 8 }} /> Place Order</>}
        </button>
      </div>
    </div>
  );
